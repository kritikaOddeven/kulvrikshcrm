...
@section('pagetitle', 'View-Email | Kulvriksh')
@section('admin-content')
    ...
    <div class="row py-3 align-items-sm-center justify-content-between gap-2">
        ...
        <div class="mt-3 mt-sm-0 col-auto">
            @if (!auth()->user()->hasImapConfigured())
                <a href="{{ url('admin/settings/my-imap') }}" class="btn btn-primary">
                ...
                <div class="card">
                    <div class="card-body p-0">
                        <div class="list-group list-group-flush rounded-2" id="folders-list">
                            <div class="list-group-item folder-item {{ $folder === 'INBOX' ? 'active' : '' }}" data-folder="INBOX">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div><i class="ri-inbox-line me-2"></i> Inbox</div>
                                    <span class="badge bg-primary rounded-pill" id="inbox-count">{{ $emailCounts['INBOX'] ?? 0 }}</span>
                                </div>
                            </div>
                            <div class="list-group-item folder-item {{ $folder === 'SENT' ? 'active' : '' }}" data-folder="SENT">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div><i class="ri-send-plane-fill me-2"></i> Sent</div>
                                    <span class="badge bg-secondary rounded-pill" id="sent-count">{{ $emailCounts['SENT'] ?? 0 }}</span>
                                </div>
                            </div>
                            {{-- <div class="list-group-item folder-item {{ $folder === 'DRAFTS' ? 'active' : '' }}" data-folder="DRAFTS">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div><i class="ri-file-edit-line me-2"></i> Drafts</div>
                                    <span class="badge bg-warning rounded-pill" id="drafts-count">{{ $emailCounts['DRAFTS'] ?? 0 }}</span>
                                </div>
                            </div> --}}
                            <div class="list-group-item folder-item {{ $folder === 'STARRED' ? 'active' : '' }}" data-folder="STARRED">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div><i class="ri-star-fill me-2"></i> Starred</div>
                                    <span class="badge bg-warning rounded-pill" id="starred-count">{{ $emailCounts['STARRED'] ?? 0 }}</span>
                                </div>
                            </div>
                            <div class="list-group-item folder-item {{ $folder === 'TRASH' ? 'active' : '' }}" data-folder="TRASH">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div><i class="ri-delete-bin-line me-2"></i> Trash</div>
                                    <span class="badge bg-danger rounded-pill" id="trash-count">{{ $emailCounts['TRASH'] ?? 0 }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col d-flex justify-content-between align-items-center">
                                <h4 class="card-title mb-0" id="current-folder">{{ ucfirst(strtolower($folder)) }}</h4>
                                <div class="d-flex align-items-center gap-2">
                                    <input type="text" class="form-control form-control-sm" id="search-input" placeholder="Search emails" style="width: 200px;" value="{{ $search }}">
                        ...
                        <div id="email-list">
                            @if ($emails->count() > 0)
                                @foreach ($emails as $email)
                                    ...
                                    // die;
                                    ?>
                                    <div class="email-item list-group-item {{ !$email->is_read ? 'unread' : '' }}" data-email-id="{{ $email->id }}" onclick="viewEmailDetails({{ $email->id }}, event)">
                                        <div class="row align-items-center p-2 px-3">
                                            <div class="col-auto">
                                                <input type="checkbox" class="email-checkbox" value="{{ $email->id }}" onclick="event.stopPropagation()">
                                            </div>
                                            <div class="col-auto">
                                                <i class="ri-star-{{ $email->is_starred ? 'fill star-icon' : 'line' }}" onclick="toggleStar({{ $email->id }}, this); event.stopPropagation()"></i>
                                            </div>
                                            <div class="col">
                                                <div class="d-flex justify-content-between align-items-start">
                                                    <div class="flex-grow-1">
                                                        <div class="fw-semibold text-truncate">{{ $email->from_name ?: $email->from_email }}</div>
                                                        <div class="text-muted small text-truncate text-wrap-400">{{ $email->to_name }}</div>
                                                        <div class="text-muted small text-truncate text-wrap-400">{{ $email->subject }}</div>

                                                        {{-- <div class="text-muted small text-truncate">{{ $email->short_body }}</div> --}}
                                                    </div>
                                                    <div class="text-end">
                                                        <div class="text-muted small">{{ $email->formatted_date }}</div>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-md-6 text-end">
                                <div class="d-flex align-items-center justify-content-end gap-2">
                                    <span id="pagination-info" class="pagination-info">{{ $emails->firstItem() ?? 0 }}-{{ $emails->lastItem() ?? 0 }}/{{ $emails->total() ?? 0 }}</span>
                                    <div class="btn-group" role="group">
                                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="previousPage()" id="prev-btn" {{ $emails->previousPageUrl() ? '' : 'disabled' }}>
                                            <i class="ri-arrow-left-s-line"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="nextPage()" id="next-btn" {{ $emails->nextPageUrl() ? '' : 'disabled' }}>
    ...
    <script>
        ...
        let selectedEmails = [];
        let totalPages = {{ $emails->lastPage() }};
        let totalRecords = {{ $emails->total() }};
        ...
        function refreshEmails() {
            ...
            fetch('{{ url('admin/mass-email/view-email/refresh') }}', {
                    method: 'POST',
                    headers: {
        ...
        function markSelectedAsRead() {
            ...
            fetch('{{ url('admin/mass-email/view-email/mark-read') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function viewEmailDetails(emailId, event) {
            ...
            window.location.href = `{{ url('admin/mass-email/view-email/show') }}/${emailId}`;
        }
