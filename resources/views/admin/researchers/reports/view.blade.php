...
@section('pagetitle', 'View Report | Kulvriksh')
    ...
    <div class="row py-3 align-items-center justify-content-between gap-2">
        <div class="col-auto">
            ...
            <ol class="breadcrumb m-0 py-0">
                <li class="breadcrumb-item"><a href="{{ url('admin/researcher') }}">Researcher</a></li>
                <li class="breadcrumb-item active"><a href="{{ url('admin/research-reports') }}">View All Researcher Report</a></li>
                <li class="breadcrumb-item active">View Research Report</li>
            </ol>
        </div>
        @if (!$data->deleted_at)
        <div class="d-flex flex-wrap gap-2 col-auto">
            @can('send_researcher_report_whatsapp')
                <a href="" type="button" class="whatsapp-icon-btn"><i class="ri-whatsapp-line"></i></a>
            @endcan

            @can('send_researcher_report_email')
                @livewire('send-lead-email', ['lead' => $data->lead], key($data->lead->id))
            @endcan

            @can('preview_researcher_report')
                @if ($data->report && $data->report->exists())
                    <a href="{{ url('admin/research-reports/pdf1/' . $data->id) . '?preview=true' }}" type="button" data-bs-toggle="tooltip" title="preview" class="btn btn-success"><i class="ri-printer-line"></i> Preview</a>
                @endif
            @endcan

            @can('fill_researcher_report_form')
                <a href="{{ url('admin/research-reports/edit/' . $data->id) }}" class="btn btn-primary mr-1" data-bs-toggle="tooltip" title="edit"><i class="ri-edit-line"></i> Fill Form</a>
            @endcan
        </div>
        @endif
    </div>
